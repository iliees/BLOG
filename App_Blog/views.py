...
class CreateBlog(LoginRequiredMixin,CreateView):
    ...
    def form_valid(self, form):
        ...
        # Generate a slug using slugify
        slug = slugify(title)
        # Check for any existing slugs with the same value
        existing_slugs = Blog.objects.filter(slug__startswith=slug)
        if existing_slugs.count() > 0:
            slug = f"{slug}-{existing_slugs.count() + 1}"  # Append a number to make it unique
        blog_obj.slug = slug
        blog_obj.save()
